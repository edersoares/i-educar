...
--
-- Name: media_final_rondon(integer, integer); Type: FUNCTION; Schema: relatorio; Owner: ieducar
--

CREATE OR REPLACE FUNCTION media_final_rondon(matricula_id_ integer, componente_id integer) RETURNS numeric
    LANGUAGE plpgsql
AS $$
DECLARE
    nota1 numeric;
...
BEGIN
    ...
    nota1 :=
        ...
        AND (nota1 < nota3)
        AND (nota1 <= nota_exame) THEN RETURN TRUNC((((nota_exame * 3) + (nota2 * 3) + (nota3 * 4))/10),1); END IF; END IF; RETURN relatorio.media_anual_rondon(matricula_id_, componente_id); END; $$;
