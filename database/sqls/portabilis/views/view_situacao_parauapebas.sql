...
--
-- Name: view_situacao_parauapebas; Type: VIEW; Schema: relatorio; Owner: postgres
--

CREATE OR REPLACE VIEW view_situacao_parauapebas AS
SELECT matricula.cod_matricula,
       ...
       ( SELECT
             CASE
                 ...
                 WHEN (matricula_turma.abandono OR (matricula.aprovado = 6)) THEN
                     CASE
                         WHEN (matricula.ref_cod_abandono_tipo IS NOT NULL) THEN ( SELECT abandono_tipo.nome
                                                                                   FROM pmieducar.abandono_tipo
                                                                                   WHERE (abandono_tipo.cod_abandono_tipo = matricula.ref_cod_abandono_tipo))
                         ELSE 'Abandono'::character varying
                         END
...
WHERE ((matricula.ativo = 1) AND
       ...
       CASE
           ...
           WHEN (situacao_matricula.cod_situacao = ANY (ARRAY[1, 2, 3, 12, 13])) THEN ((matricula.aprovado = situacao_matricula.cod_situacao) AND ((NOT matricula_turma.reclassificado) OR (matricula_turma.reclassificado IS NULL)) AND ((NOT matricula_turma.abandono) OR (matricula_turma.abandono IS NULL)) AND ((NOT matricula_turma.remanejado) OR (matricula_turma.remanejado IS NULL)) AND ((NOT matricula_turma.transferido) OR (matricula_turma.transferido IS NULL)) AND ((NOT matricula_turma.falecido) OR (matricula_turma.falecido IS NULL)))
